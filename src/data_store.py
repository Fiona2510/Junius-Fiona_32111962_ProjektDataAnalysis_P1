...
def bootstrap_from_csv(csv_path: str, db_path: str = DEFAULT_DB, table: str = DEFAULT_TABLE, overwrite: bool = False, make_parquet: bool = True) -> None:
    con = connect(db_path)
    if overwrite:
...
def get_text_and_rating(db_path: str = DEFAULT_DB, table: str = DEFAULT_TABLE) -> pd.DataFrame:
    df_cols = query_df(f"PRAGMA table_info({table});", db_path)
    cols = {row['name'].lower(): row['name'] for _, row in df_cols.iterrows()}
    ...
    sel_cols = [text_col] + ([rating_col] if rating_col else [])
    query = f"SELECT {', '.join(sel_cols)} FROM {table} WHERE {text_col} IS NOT NULL;"
    return query_df(query, db_path)
